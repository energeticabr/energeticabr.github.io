...
import { createOperationsAssistant } from "./ui/operations-assistant.js?v=20260909-presence-accordion-v1";
...
export function renderModuleLanding(container, moduleId, options = {}) {
  ...
  const entityPair = (className, id, label, { createOnly = false, launchTarget = id } = {}) => {
    ...
    const commands = `${suppliesCommand(id, true, launchTarget)}${createOnly ? "" : suppliesCommand(id)}`;
    if (!commands) return "";
    return `<article class="${className}${createOnly ? " supplies-action-single" : ""}"><h3>${escapeHtml(label)}</h3><div class="module-entity-actions">${commands}</div></article>`;
  };
  const suppliesPair = (id, label, createOnly = false, launchTarget = id) => entityPair(
    "supplies-action-row",
    id,
    label,
    { createOnly, launchTarget },
  );
  const remainingRows = className => entities.map(entity => {
    if (renderedCommands.has(`${entity.id}:gallery`)) return "";
  ...
  if (moduleId === "comercial" && !options.entities) {
    ...
    const pair = ([id, label]) => entityPair("commercial-action-row", id, label);
    const registrationRows = registrations.map(pair).join("");
    const operationRows = operations.map(pair).join("");
    const homologationRow = pair(homologation);
    const remaining = remainingRows("commercial-action-row");
    container.innerHTML = `<section class="module-page commercial-module-page" aria-labelledby="moduleTitle"><header class="module-heading commercial-module-heading"><div><p class="page-eyebrow">I7 · GERAL COMERCIAL</p><h1 id="moduleTitle">Comercial</h1><p class="module-page-intro">Cadastros, receitas e atendimento comercial em um fluxo simples.</p></div></header><div class="commercial-workspace"><section class="commercial-column commercial-catalog" aria-labelledby="commercialCatalogTitle"><div class="commercial-column-heading"><span class="commercial-column-kicker">Cadastros</span><h2 id="commercialCatalogTitle">Bases comerciais</h2><p>Cadastre e consulte clientes, imóveis, corretores e classificações.</p></div><div class="commercial-action-list">${registrationRows}</div></section><section class="commercial-column commercial-operations" aria-labelledby="commercialOperationsTitle"><div class="commercial-column-heading"><span class="commercial-column-kicker">Operação</span><h2 id="commercialOperationsTitle">Receitas e atendimento</h2><p>Registre receitas e acompanhe apontamentos comerciais e solicitações do SAC.</p></div><div class="commercial-action-list">${operationRows}${remaining}</div><div class="commercial-homologation">${homologationRow}</div></section></div></section>`;
    return;
  }
  if (moduleId === "rh-obras" && !options.entities) {
    ...
    const pair = ([id, label]) => entityPair("hr-action-row", id, label);
    const peopleRows = people.map(pair).join("");
    const operationRows = operations.map(pair).join("");
    const contractRows = contracts.map(pair).join("");
    const remaining = remainingRows("hr-action-row");
    container.innerHTML = `<section class="module-page hr-module-page" aria-labelledby="moduleTitle"><header class="module-heading hr-module-heading"><div><p class="page-eyebrow">I6 · GERAL RH</p><h1 id="moduleTitle">Recursos Humanos e Obras</h1><p class="module-page-intro">Pessoas, presença, tarefas e acompanhamento da execução.</p></div></header><div class="hr-workspace"><section class="hr-column hr-people" aria-labelledby="hrPeopleTitle"><div class="hr-column-heading"><span class="hr-column-kicker">Cadastros</span><h2 id="hrPeopleTitle">Pessoas e registros</h2><p>Cadastre informações de apoio e consulte os registros já existentes.</p></div><div class="hr-action-list">${peopleRows}</div></section><section class="hr-column hr-operations" aria-labelledby="hrOperationsTitle"><div class="hr-column-heading"><span class="hr-column-kicker">Execução</span><h2 id="hrOperationsTitle">Presença, tarefas e obras</h2><p>Registre atividades e acompanhe a rotina da obra.</p></div><div class="hr-action-list">${operationRows}</div><div class="hr-contracts"><h2>Contratos e medições</h2><div class="hr-action-list">${contractRows}${remaining}</div></div></section></div></section>`;
    return;
  }
  if (moduleId === "patrimonio-locacoes" && !options.entities) {
    ...
    const operationRows = operations.map(pair).join("");
    const registrationRows = registrations.map(pair).join("");
    const remaining = remainingRows("property-action-row");
    container.innerHTML = `<section class="module-page property-module-page" aria-labelledby="moduleTitle"><header class="module-heading property-module-heading"><div><p class="page-eyebrow">SCREEN1 · PATRIMÔNIO E LOCAÇÕES</p><h1 id="moduleTitle">Patrimônio e Locações</h1><p class="module-page-intro">Imóveis, contratos, pagamentos e documentação organizados em um só lugar.</p></div></header><div class="property-workspace"><section class="property-column property-operations" aria-labelledby="propertyOperationsTitle"><div class="property-column-heading"><span class="property-column-kicker">Operação</span><h2 id="propertyOperationsTitle">Pagamentos e homologações</h2><p>Trate pendências e consulte as galerias correspondentes.</p></div><div class="property-action-list">${operationRows}</div></section><section class="property-column property-catalog" aria-labelledby="propertyCatalogTitle"><div class="property-column-heading"><span class="property-column-kicker">Cadastros</span><h2 id="propertyCatalogTitle">Imóveis e locações</h2><p>Cadastre e consulte os registros usados na administração patrimonial.</p></div><div class="property-action-list">${registrationRows}${remaining}</div></section></div></section>`;
    return;
  }
  if (moduleId === "auditoria-compliance" && !options.entities) {
    ...
    const pair = ([id, label]) => entityPair("audit-action-row", id, label);
