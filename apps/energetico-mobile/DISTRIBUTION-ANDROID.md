...
## Publicação automática no teste interno

O workflow `Energético Android (APK de teste)` gera o AAB em cada push para `main`. Depois que a credencial da API da Google Play estiver configurada, ele também publica automaticamente o AAB na faixa de testes internos.

1. No Google Cloud, habilite a Google Play Developer API e crie uma conta de serviço.
2. No Play Console, convide o e-mail dessa conta em **Usuários e permissões** e conceda a permissão **Liberar apps em faixas de teste** para o Energético.
...
4. Faça um push para `main`. O workflow testa, assina, gera o AAB e publica a nova versão em **Testes internos**.

O envio automático permanece na faixa interna para que cada versão possa ser validada antes de chegar à produção. O workflow manual também aceita `build_play_aab=true` e `publish_play=true` para gerar e publicar uma versão sob demanda.
