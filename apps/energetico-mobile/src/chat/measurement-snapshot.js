...
export function normalizeMeasurementSnapshot(value) {
  ...
  const fields = ["activity", "quantity", "height", "width", "unitPrice", "unitPriceDisplay", "total", "totalDisplay"];
  if (value.lines.some(line => !line || !Number.isInteger(line.index) || line.index < 1
    || fields.some(key => typeof line[key] !== "string")
    || ["unitPrice", "total"].some(key => !decimal(line[key])))) return undefined;
  return Object.freeze({ id: value.id, currency: "BRL", count: value.count, total: value.total,
    totalDisplay: value.totalDisplay, lines: Object.freeze(value.lines.map(line => Object.freeze({
      ...
      attachmentCount: Number.isFinite(Number(line.attachmentCount)) ? Number(line.attachmentCount) : 0,
      details: Object.freeze(Object.fromEntries(["contract", "branch", "description", "unit", "lineType"].map(key => [key, String(line.details?.[key] || "Em branco")]))),
      ...Object.fromEntries(["editReply", "deleteReply"].filter(key => typeof line[key] === "string" && /^measurement_line:(edit|delete):[a-f0-9]{24}:\d+$/.test(line[key])).map(key => [key, line[key]])),
    })))});
}
