...
5. O workflow `Energético iOS` não envia mais builds automaticamente após alterações na `main`. Para publicar no TestFlight, execute-o manualmente com `distribute=true`; ele fará os testes, a compilação assinada, o envio e aguardará a build ficar válida para associá-la ao grupo interno `ENERGETICO Validacao`. A opção `attach_latest_testflight=true` associa uma build já enviada sem gerar outra. Para administrar os testadores desse grupo sem gerar build, use `sync_testflight_testers=true` e informe `testflight_allowed_emails`. O sincronizador convida automaticamente um endereço que ainda não é usuário do App Store Connect, com acesso de Developer somente ao Energético; a pessoa precisa aceitar o convite e a rotina deve ser executada novamente para concluir a troca e remover os demais.
...
O job de distribuição só roda em uma execução manual com `distribute=true`; ele não roda em `pull_request` nem em pushes. A execução manual usa o ambiente protegido `app-store-connect`; se houver um revisor obrigatório, fica aguardando essa aprovação antes de assinar, enviar ou associar qualquer build. Sem todas as credenciais preexistentes, falha antes de arquivar ou enviar qualquer build.
