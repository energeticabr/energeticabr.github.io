...
function normalizedCanvasFileName(fileName) {
  return decodeLegacyPercentFileName(fileName)
...
function sourceCodeComponentDefinition(line) {
  ...
  const typeMatch = declaration.match(/^([A-Za-z][A-Za-z0-9_]*)(?:\.(.+))?$/);
  if (!typeMatch) return null;
  const rawType = typeMatch[1];
  const variant = String(typeMatch[2] || "").replace(/^'|'$/g, "") || null;
  const control = rawType.toLocaleLowerCase("en-US") === "gallery"
    ? "Gallery"
    : rawType.toLocaleLowerCase("en-US") === "screen"
      ? "Screen"
...
function parsePowerAppsYaml(content, fileName) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (sourceCodeDefinition) {
      ...
      if (control === "Screen") screenName = name;
      const component = {
        fileName,
        screenName,
        name,
        ...
        properties: {
          Control: { lineNumber: index + 1, literal: control },
          ...(variant ? { Variant: { lineNumber: index + 1, literal: variant } } : {}),
        },
      };
...
export function extractPowerAppsGalleryContracts(files, options = {}) {
  ...
  return {
    schemaVersion: 1,
    source: {
      inventory: String(options.inventory || SOURCE_LABEL).replace(/\\/g, "/"),
...
export async function extractPowerAppsGalleryContractsFromDirectory(sourceDir) {
  ...
  const inventory = resolve(sourceDir) === resolve(DEFAULT_SOURCE_DIR)
    ? SOURCE_LABEL
    : resolve(sourceDir).replace(/\\/g, "/");
  return extractPowerAppsGalleryContracts(files, { inventory });
}

export function renderPowerAppsGalleryContracts(result) {
  return [
    "// Generated by scripts/generate-powerapps-gallery-contracts.mjs.",
    `// Source: ${result?.source?.inventory || SOURCE_LABEL}`,
    "",
...
export function renderPowerAppsGalleryFieldParity(result) {
  ...
  const lines = [
    ...
    "## Fonte e cobertura",
    "",
    `- Fonte examinada: \`${String(source.inventory || SOURCE_LABEL).replace(/\//g, "\\")}\`.`,
    `- Foram lidos ${source.fileCount} arquivos de tela Canvas; ${source.galleryCount} Galleries aparecem em ${source.screenCount} telas/arquivos com Gallery.`,
